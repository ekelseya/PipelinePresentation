...
\begin{figure}
    \centering
    \includegraphics[width=3.0in]{viewingpyramid.gif}
    \caption{The viewing pyramid}
    \label{viewingpyramid}
\end{figure}

The camera transformation is a combination of two transformations: first, the world coordinates are transformed to local coordinates, then the viewing transform is applied.

\paragraph
If the camera is at the origin, the viewing direction is alonng the negative $w$ axis. Points are projected along the line that passes through the origin onto a plane defined by $w = -d$.

\begin{figure}
    \centering
    \includegraphics[width=3.0in]{projectiontriangle.gif}
    \caption{The projection of a point onto the plane}
    \label{projectiontriangle}
\end{figure}

Note: $v' = (\frac{d}{-w})v$ and $u' = (\frac{d}{-w})u$.

The transformation is expressed in homogenous coordinates by 
...
This unique fourth column gives the distance of the point from the $xy$ plane.
